...
cd /builds/buildbot/pmoore/staging/staging
# change from symbolic link to physical file, so that changes are not lost in a reconfig...
rm Makefile
cp buildbot-configs/Makefile.master Makefile
# now make changes
echo -e 'reset-db:\n\trm master/state.sqlite\n\tcd master && $(BUILDBOT) upgrade-master $$PWD' >> Makefile
echo -e 'clobber:\n\trm -f master/twistd.log*\n\tfor f in $$(find master -name builder); do rm -rf $$(dirname $$f); done' >> Makefile
cp ~/passwords.py master/passwords.py
make start
